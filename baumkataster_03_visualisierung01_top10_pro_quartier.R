...
g <- ggplot(top10_quartier, aes(fill=baumnamedeu_kompakt, y = n, x = quartier)) +
  geom_bar(position = "stack", stat = "identity", width = 0.6) +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank(), panel.grid.major = element_blank(), axis.title.y = element_blank()) +
  geom_text(aes(label = n), size = 3, position = position_stack(vjust = 0.5)) +
  labs(title =paste("Anzahl vorhandene Bäume in", quartier_name), fill = "Baumname Deutsch", subtitle = paste("n =", sum(top10_quartier$n)))
